...
#region Singleton
	...
	public static SoundManager Instance
	{
		get
		{
			if (instance == null)
			{
				Object soundManagerPrefab = Resources.Load("SoundManager");
				
				if(soundManagerPrefab == null)
				{
					Debug.LogError("SoundManager prefab must exist in Resources folder");
					return null;
				}
				else
				{
					instance 		= (Instantiate(soundManagerPrefab) as GameObject).GetComponent<SoundManager>();
	...
	void Awake()
	{
		...
		if(PlayerPrefs.HasKey("SoundManagerVolume"))
		{
			SetVolume(PlayerPrefs.GetFloat("SoundManagerVolume"));
		}
		else
...
#region 3D methods
	...
	public void SetVolume(float _volume)
	{
		...
		if(cachedAudioSources.Count == 0)
			Debug.LogError("You must call PrepareAudioSources int he Awake or Start of your script before work with SoundManager");
	...
	{
		PlayerPrefs.SetFloat("SoundManagerVolume",_newVolume);	
